...
const Button: React.FC<ButtonProps> = ({
  ...
  // Base styles
  let baseStyles = 'font-medium rounded-full transition-all duration-200 flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-offset-2';
  ...
  return (
    <button
      className={`${combinedStyles} ${isDisabled ? 'opacity-60 cursor-not-allowed' : ''}`}
      disabled={isDisabled}
      {...props}
    >
      {isLoading && (
